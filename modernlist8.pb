    ...
    If totalFlex > 0
      ...
      ForEach elements()
        If elements()\Width = 0 And elements()\Flex > 0
          calculatedWidths(index) = Round((elements()\Flex / totalFlex) * remainingWidth, #PB_Round_Down)
        EndIf
        index + 1
      Next
    EndIf
  ...
  Procedure DrawHeader(*list.ModernListData)
    If *list\HeaderContainer
      If StartDrawing(CanvasOutput(*list\HeaderContainer))
        ...
        CalculateElementWidths(*list\Header\Columns(), *list\Width, calculatedWidths())
        ...
        ForEach *list\Header\Columns()
          colWidth = calculatedWidths(colIndex) * Local_DPI_Scale
  ...
  Procedure DrawRow(*list.ModernListData, rowIndex.i, gadget.i, active.i, hovered.i)
    ...
    If gadget And IsGadget(gadget)
      ...
      If StartDrawing(CanvasOutput(gadget))
        ...
        CalculateElementWidths(*list\Rows()\Elements(), *list\Width, calculatedWidths())
        
        xPos = 0
        Protected elemIndex.i = 0
        
        ForEach *list\Rows()\Elements()
          elemWidth = calculatedWidths(elemIndex) * Local_DPI_Scale
          If StartDrawing(CanvasOutput(gadget))
            DrawingMode(#PB_2DDrawing_Transparent)
            ...
            b = Random(255)
            Box(xPos, 0, elemWidth, canvasH, RGB(r, g, b))
            StopDrawing()
              ...
              If StartDrawing(CanvasOutput(gadget))
                ...
                If Val(*list\Rows()\Elements()\Content) > 0
                  ...
                  If tintColor <> 0
                    ...
                    Box(xPos, 0, elemWidth*DPI_Scale, canvasH, tintColor)
                  EndIf
                EndIf
                StopDrawing()
  ...
  Procedure CreateList(window.i, x.i, y.i, width.i, height.i, List rows.ListRow(), *header.ListHeader=0, defaultRowHeight.i=30, User_DPI_Scale.f=1, *resizeProc=0)
    ...
    *list\InnerContainer = ContainerGadget(#PB_Any, 0, 0, *list\Width, totalRowHeight, #PB_Container_BorderLess)
    ...
    ForEach *list\Rows()
      ...
      CalculateElementWidths(*list\Rows()\Elements(), *list\Width, elemWidths())
...
Procedure Main()
  ...
  If OpenWindow(0, 0, 0, 750, 500, "ModernList Example with Flex Layout - DPI: " + StrF(DPI_Scale * 100, 0) + "%", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    ...
    Protected *list.ModernListData = ModernList::CreateList(0, 0, 0, 750, 500, rows(), @header, 30, DPI_Scale, @ListResizeCallback())
