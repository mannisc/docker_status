...
DeclareModule App
  ...
  Declare WebViewResizeGadget(Gadget, x.f, y.f, width.f, height.f, parentsRoundingDeltaX.f = 0, parentsRoundingDeltaY.f = 0)
  
  Declare.s RGBToHex(color)
    ...
    CompilerIf #PB_Compiler_OS = #PB_OS_Windows
      ...
      Select msg
        ...
        Case #WM_CTLCOLORBTN
          ...
          Case #WM_ERASEBKGND
      ; CRITICAL: Paint dark background here
      hdc = wParam
      GetClientRect_(hwnd, @rect)

      ; Fill with dark color
       hBrush = CreateSolidBrush_(RGB(30, 30, 50))
      FillRect_(hdc, @rect, hBrush)
      DeleteObject_(hBrush)

      ProcedureReturn 1  ; We erased

    Case #WM_PAINT
      ; Let default paint (for child controls), then we already erased
  ...
  Procedure DoResize(*tabBar.VerticalTabBarData,externalResize = #False)
    ...
    *tabBar\ParentsRoundingDeltaX = parentsRoundingDeltaX
    ...
    If *tabBar\ResizeCallback
      CallFunctionFast(*tabBar\ResizeCallback, *tabBar, *tabBar\ActiveTabIndex, newContentWidth,*tabBar\ContentHeight, *tabBar\IsExpanded,@sidebarDifference,@parentsRoundingDeltaX )
    EndIf
...
;         SendMessage_(GadgetID(*tabBar\ContentContainer), #WM_SETREDRAW, #True, 0)
    ...
    Procedure HandleLayout(*tabBar.VerticalTabBarData, index.i, width, height, IsExpanded,*deltaWidth, *parentsRoundingDeltaX)
      ...
      NormalResizeGadget(tabIds(index), #PB_Ignore, #PB_Ignore, width, #PB_Ignore,parentsRoundingDeltaX)
          ...
          Select index 
            ...
            Case 4:  ;"Flow" 
              ResizeGadget(*Gadgets\FlowWebView,#PB_Ignore, #PB_Ignore,width, height-buttonAreaHeight)
          EndSelect
        ...
        If OpenWindow(#WIN_ID, 0, 0, windowWidth , windowHeight , "Monitor - VS Code Style", 
          ...
          *Gadgets\FlowWebView= WebViewGadget(#PB_Any,0,0, width, height,#PB_WebView_Debug)
          ...
          Protected htmlContent.s = "" +
                          ...
                          ~"      height: 100vh;\n" +
                          ~"      overflow: hidden;\n" +
                          ~"      background: #"+RGBToHex(themeBackgroundColor)+~";\n" +
                          ...
                          ~"        const element = document.getElementById('centerElement');\n" +
                          ~"        if (element) {\n" +
  ...
  Module AppWindow
    ...
    Procedure HandleLayout(*tabBar.VerticalTabBarData, index.i, width, height, IsExpanded,*deltaWidth,*parentsRoundingDeltaX)
      ...
      Select index 
        Case 0:       
          ModernList::ResizeList(*list, width-10, height-buttonAreaHeight-5)
    ...
    Procedure CreateButtonGadget(parentGadget.i,content.s, x.i, y.i, width.i, height.i)
      
      Protected g.i = ButtonGadget(#PB_Any, x, y, width, height, content)
      If IsGadget(g)
        If IsDarkModeActiveCached
    ...
    Procedure CreateStringGadget(parentGadget.i,content.s, x.i, y.i, width.i, height.i)
    ...
    #ICON_OVERLAY_SIZE = 22
    Procedure CreateListMockImage()
       w = #ICON_OVERLAY_SIZE
    h = #ICON_OVERLAY_SIZE
      Protected img.i = CreateImage(#PB_Any, w, h,32,#PB_Image_Transparent )
      
      
      StartVectorDrawing(ImageVectorOutput(img))
      VectorSourceColor(RGBA(255, 255, 255, 255))
      AddPathEllipse(11, 11,11,11)  
      FillPath()
      VectorSourceColor(RGBA(255,0,0, 255))
      AddPathEllipse(11, 11,6,6)      ; width=16, height=16
      FillPath()
      StopVectorDrawing()
...
;         Circle(w/2, h/2, w/2- #ICON_OVERLAY_SIZE*0.1, RGB(255,255,100))
;         StopDrawing()
;         StopDrawing()
;       EndIf
      ProcedureReturn img
      ...
      If Not IsWindow(#WIN_ID)
        ...
        If OpenWindow(#WIN_ID, 0, 0, windowWidth , windowHeight , "Monitor - VS Code Style", 
          ...
          *monitorTabBar = *tabBar
          ...
          header\Columns()\Content = "Monitor"
          header\Columns()\Flex = 1
         ...
         ; rows()\Elements()\SelectedHoveredTintColor = RGBA(200, 200, 200, 50)
          ...
          rows()\Elements()\Content = "Angular"
          rows()\Elements()\Flex = 1

          rows()\Elements()\Alignment = 0
          rows()\Elements()\SelectedForeColor = RGB(235, 235, 235)
          rows()\Elements()\SelectedHoveredForeColor = RGB(255, 255, 255)
          rows()\Elements()\HoveredForeColor = RGB(255, 255, 255)
          
          AddElement(rows()\Elements())
          rows()\Elements()\Type = ModernListElement::#Element_Gadget
          rows()\Elements()\Content = "Start"
          rows()\Elements()\Interface = @ButtonInterface
          rows()\Elements()\Width = 50
          
          AddElement(rows()\Elements())
          rows()\Elements()\Type = ModernListElement::#Element_Space
          rows()\Elements()\Width = 5
          
          AddElement(rows()\Elements())
          rows()\Elements()\Type = ModernListElement::#Element_Gadget
          rows()\Elements()\Content = "Edit"
          rows()\Elements()\Interface = @ButtonInterface
          rows()\Elements()\Width = 50
          Next 
...
;           Next
    ...
    Procedure.i HandleEvent(Event.i, Gadget.i) 
      ...
      Static lastMouseX.i = -1
      Static lastMouseY.i = -1
      
      
      Protected currentMouseX.i = WindowMouseX(#WIN_ID)
      Protected currentMouseY.i = WindowMouseY(#WIN_ID)
      ...
      If Not ModernList::HandleListEvent(*list, Gadget, EventType)
