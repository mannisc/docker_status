; [App and ModernListElement modules remain unchanged from your provided code]
DeclareModule App
...
Module ModernList
  ...
  Procedure CreateList(window.i, x.i, y.i, width.i, height.i, List rows.ListRow(), *header.ListHeader=0, defaultRowHeight.i=40, User_DPI_Scale.f=1, *resizeProc=0)
    ...
    *list\MainContainer = ContainerGadget(#PB_Any, x, y, *list\Width, *list\Height, #PB_Container_BorderLess)
    ...
    ForEach *list\Rows()
      ...
      totalRowHeight + rowH
    Next
    *list\ScrollArea = ScrollAreaGadget(#PB_Any, 0, scrollY, *list\Width, scrollH, *list\Width, totalRowHeight, *list\RowHeight, #PB_ScrollArea_BorderLess)
  ...
  Procedure HandleListEvent(*list.ModernListData, eventGadget.i, event.i)
    ...
    ; Check if eventGadget is a row canvas or a child gadget
    For i = 0 To ArraySize(*list\rowGadgets())
    ...
    ; If not a row canvas, check if it's a child gadget and find parent row
    If rowIndex = -1
    ...
    If rowIndex >= 0 And rowIndex < ListSize(*list\Rows())
      Select event
        Case #PB_EventType_LeftClick
          ; Only select row if clicking directly on the canvas, not a child gadget
