...
Module ModernListElement
  ...
  Procedure HandleEvent(currentElementMap.s,foundRowGadget, foundCellGadget, foundChildGadget, childIndex, eventGadget.i, event.i, *list, index)
    Debug "###HandleEvent"
    Debug "foundRowGadget"
    Debug foundRowGadget
    Debug "foundCellGadget"
    Debug foundCellGadget
    Debug "foundChildGadget"
    Debug foundChildGadget
                    Debug "childIndex "+Str(childIndex)
  ...
  Procedure.i IsBelowMousePointer(hwnd.i)
    ...
    CompilerSelect #PB_Compiler_OS
      ...
      CompilerCase #PB_OS_MacOS
        ImportC "/System/Library/Frameworks/CoreGraphics.framework/CoreGraphics"
          CGWindowListCopyWindowInfo(option.l, relativeToWindow.l) As "CGWindowListCopyWindowInfo"
          CFRelease(cf)
          CFDictionaryGetValueIfPresent(dict, key, *value)
          CFNumberGetValue(num, type, *value)
          EndImportC
          
          Protected pt.POINT
          GetCursorPos_(@pt)
          
          Protected Array = CGWindowListCopyWindowInfo(#kCGWindowListOptionOnScreenOnly, #kCGNullWindowID)
          If Array = 0
            ProcedureReturn #False
          EndIf
          
          Protected count = CocoaMessage(0, Array, "count")
          Protected found = #False
          
          For i = 0 To count - 1
            Protected dict = CocoaMessage(0, Array, "objectAtIndex:", i)
            If dict
              Protected layer = 0
              CocoaMessage(@layer, dict, "valueForKey:$", @"kCGWindowLayer")
              If layer < 0
                Continue
              EndIf
              
              Protected bounds = CocoaMessage(0, dict, "valueForKey:$", @"kCGWindowBounds")
              Protected x.f, y.f, w.f, h.f
              CocoaMessage(@x, bounds, "valueForKey:$", @"X")
              CocoaMessage(@y, bounds, "valueForKey:$", @"Y")
              CocoaMessage(@w, bounds, "valueForKey:$", @"Width")
              CocoaMessage(@h, bounds, "valueForKey:$", @"Height")
              
              Protected screenH = DesktopHeight(0)
              y = screenH - y - h
              
              If pt\x >= x And pt\x < x + w And pt\y >= y And pt\y < y + h
                Protected windowID = 0
                CocoaMessage(@windowID, dict, "valueForKey:$", @"kCGWindowNumber")
                Protected nsWindow = CocoaMessage(0, 0, "NSWindow windowWithWindowNumber:", windowID)
                If nsWindow
                  Protected pbHwnd = CocoaMessage(0, nsWindow, "windowRef")
                  If pbHwnd = hwnd
                    found = #True
                    Break
                  EndIf
            ...
            ElseIf ElementMap()\ChildGadgets <> 0
              ...
              ForEach ElementMap()\ChildGadgets\Gadgets()
                ...
                If ElementMap()\ChildGadgets\Gadgets() = eventGadget
                  ...
                  Debug "FOUND ChildGadget ######################## "+ListSize(ElementMap()\ChildGadgets\Gadgets())
                  Break 
                EndIf 
                childIndex  +1 
                Debug "childIndex "+Str(childIndex)
              ...
              EndIf 
            EndIf
               Debug "foundGadget 2 : "+Str(foundGadget)

            If foundGadget
              Debug "FOUND ModernListElement ########################"
              For i = 0 To ArraySize(*list\rowGadgets())
                If *list\rowGadgets(i) = ModernListElement::ElementMap()\RowGadget
    ...
    Procedure Main()
      ...
      If OpenWindow(0, 0, 0, 750, 500, "ModernList Example - DPI: " + StrF(DPI_Scale * 100, 0) + "%", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
        ...
        Protected *list.ModernListData = ModernList::CreateList(0, 0, 0, 750, 700, rows(), @header, 30, DPI_Scale, @ListResizeCallback())
