...
DeclareModule WindowManager
  ...
  ; Structure to hold window information and its handlers
  Structure AppWindow
    WindowID.i                    ; The PureBasic window ID
    Id.i                          ; Window title for identification
    *OpenProc.ProtoOpenWindow     ; Pointer to open/init procedure
    *HandleProc.ProtoHandleEvent  ; Pointer to event handler procedure
    *CloseProc.ProtoCloseWindow   ; Pointer to cleanup procedure
    UserData.i                    ; Optional user data for the window
    Open.b                        ; Is window currently open?
    *Gadgets                      ; Map to store named gadget IDs
  EndStructure
  
  ; Public API
  Declare.i AddManagedWindow(Title.s, *Gadgets, *OpenProc, *HandleProc, *CloseProc)
...
Module WindowManager
  ...
  Procedure.i AddManagedWindow(Title.s, *Gadgets, *OpenProc, *HandleProc, *CloseProc)
    AddElement(ManagedWindows())
    ManagedWindows()\Id = Id
    ManagedWindows()\Gadgets = *Gadgets
    ManagedWindows()\OpenProc = *OpenProc
  ...
  Procedure OpenManagedWindow(*Window.AppWindow)
    If Not *Window\Open
      If *Window\OpenProc
        *Window\WindowID = CallFunctionFast(*Window\OpenProc, *Window)
  ...
  Procedure CloseManagedWindow(*Window.AppWindow)
    If *Window\WindowID
      ...
      If *Window\CloseProc
        CallFunctionFast(*Window\CloseProc, *Window\WindowID)
      EndIf
      *Window\Open = #False
  ...
  Procedure RunEventLoop()
    ...
    While KeepRunning 
      ...
      ForEach ManagedWindows()
        ; Check if this event belongs to this window
        If ManagedWindows()\Open And EventWindow = ManagedWindows()\WindowID
          ; Call the window's event handler
          If ManagedWindows()\HandleProc
            KeepWindow = CallFunctionFast(ManagedWindows()\HandleProc, 
                                          @ManagedWindows(), 
                                          Event, 
                                          EventWindow, 
                                          EventGadget)
...
Module AddMonitorDialog
  ...
  Procedure Open()
    If *Window : ProcedureReturn : EndIf ; Already open
    
    If OpenWindow(#PB_Any, 0, 0, 380, 130, "Add Container", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_Invisible)
      

          
      *Gadgets\TxtContainer = StringGadget(#PB_Any, 10, 10, 360, 24, "")
      SetActiveGadget(*Gadgets\TxtContainer)
      AddKeyboardShortcut(*Window\ID, #PB_Shortcut_Return, #EventOk)
      ...
      *Gadgets\BtnCancel = ButtonGadget(#PB_Any, 170, 100, 80, 24, "Cancel")
      ...
      StickyWindow(*Window\ID, #True)
      ApplyTheme(*Window\ID)
      Repeat : Delay(1) : Until WindowEvent() = 0
      ShowWindowFadeIn(*Window\ID)
  ...
  Procedure Close()
    If *Window
      Protected *Gadgets.AddMonitorGadgets = *Window\Gadgets
      If *Gadgets
        FreeMemory(*Gadgets)
      EndIf
  ...
  Procedure HandleEvents()
    ...
    If Window <> *Window\ID : ProcedureReturn : EndIf
...
Module EditMonitorDialog
  ...
  Procedure Open(index.i)
    If *Window : ProcedureReturn : EndIf
    
    editIndex = index
    Protected container$ = containerName(index)
    bgCol = bgColor(index)
    
    If OpenWindow(#PB_Any, 0, 0, 380, 130, "Edit Container", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_Invisible)
  ...
  Procedure Close()
    If *Window
      Protected *Gadgets.EditMonitorGadgets = *Window\Gadgets
      If *Gadgets
        FreeMemory(*Gadgets)
      EndIf
      CloseWindow(*Window\ID)
...
Module EditPatternsDialog
  ...
  Procedure UpdatePatternButtonStates()
    If Not *Window : ProcedureReturn : EndIf
    Protected *Gadgets.EditPatternsGadgets = *Window\Gadgets
    
    Protected selIndex = GetGadgetState(*Gadgets\PatternList)
    If selIndex >= 0
      DisableGadget(*Gadgets\BtnRemove, #False)
      DisableGadget(*Gadgets\BtnEdit, #False)
    Else
      DisableGadget(*Gadgets\BtnRemove, #True)
      DisableGadget(*Gadgets\BtnEdit, #True)
    EndIf
  EndProcedure
  
  Procedure UpdatePatternList()
    If Not *Window : ProcedureReturn : EndIf
    Protected *Gadgets.EditPatternsGadgets = *Window\Gadgets
    
    ClearGadgetItems(*Gadgets\PatternList)
    For p = 0 To patternCount(editIndex)-1
      AddGadgetItem(*Gadgets\PatternList, -1, "  " + patterns(editIndex, p))
      SetListItemColor(*Gadgets\PatternList, p, patternColor(editIndex, p))
    Next
  EndProcedure
  
  Procedure Open(index.i)
    If *Window : ProcedureReturn : EndIf
    If index < 0 Or index >= containerCount : ProcedureReturn : EndIf
    
    editIndex = index
    
    If OpenWindow(#PB_Any, 150, 150, 420, 450, "Edit Status Filter Rules", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_Invisible)
...
Module MainWindow
  ...
  Global *Window.AppWindow
  Global *Gadgets.MainWindowGadgets = AllocateMemory(SizeOf(MainWindowGadgets))
  ...
  Procedure Close()
    If *Window
      Protected *Gadgets.MainWindowGadgets = *Window\Gadgets
      If *Gadgets
        FreeMemory(*Gadgets)
      EndIf
      CloseWindow(*Window\ID)
    ...
    Select Event
      ...
      Case #PB_Event_Menu
        ...
        If menuEvent >= 1000
          ...
          Select menuId
            Case 0
              HideWindow(*Window\ID, #False)
              CompilerIf #PB_Compiler_OS = #PB_OS_Windows
                SetOverlayIcon(WindowID(*Window\ID), menuContainerIndex)
              CompilerEndIf
            Case 1
  ...
  *Window.AppWindow = AddManagedWindow(0,*Gadgets,@Open(),@HandleEvent(),@Close())
...
Procedure StartApp()
  ...
  OpenManagedWindow(GetWindow())
  
  ; Run the event loop (this handles all windows)
  RunEventLoop()
EndProcedure
